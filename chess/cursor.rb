...
class Cursor
  ...
  def handle_key(key)
    ...
    when :space, :return
      ...
      # if toggle_selected is false => true, @cursor_pos.save
      # if toggle_select is true => false, board.move!(saved_pos, cursor_pos)
      @cursor_pos
    when :ctrl_c
      Process.exit(0)
    end
  ...
  def update_pos(diff)
    new_pos = @cursor_pos.map.with_index do |coord, idx|
      coord + diff[idx]
    end
